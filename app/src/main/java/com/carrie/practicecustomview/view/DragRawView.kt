 ...
 */
class DragRawView(context: Context?, attrs: AttributeSet? = null) : View(context, attrs) {
        ...
        when (event?.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                if (isDraggable) {
                    ...
                    // 方式一
                    ballX = (lastBallX + dx).coerceIn(ballRadius, width - ballRadius)
                    ballY = (lastBallY + dy).coerceIn(ballRadius, height - ballRadius)
                    invalidate()
                }
            }
